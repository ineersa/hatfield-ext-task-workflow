...
Partial CODE-REVIEW failures after completed side effects (for example QA passed and the branch was pushed, but PR creation failed) keep the task in IN-PROGRESS. The tool result and task work log must identify completed steps, the failed step and bounded cause, current task path/status, available QA reports, the session/run identity, and a safe next action that does not blindly repeat already-completed steps. Credentials and raw auth output stay redacted.
